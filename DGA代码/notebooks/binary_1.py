...
# 数据文件路径
FILE = 'C:\\Users\\13719\\Desktop\\DGA\\artifacts\\binary\\dga_binary_test.csv'
df = pd.read_csv(FILE)
# 逐批读取数据的大小
chunk_size = 100
...
# 计算每批次数据的特征
# 定义函数：计算每个域名的多个特征
def calculate_features_for_chunk(chunk):
    features = []
    ...
    for domain in chunk['Domain']:
        ...
        if isinstance(domain, str):
            ...
            # 处理非字符串的域名（例如NaN等）
            features.append([0] * 22)  # 这里返回一个包含22个0的特征向量作为占位符
...
def ngrams_features_per_sample(matrix, prefix):
    ...
    for sample_frequencies in ngram_frequencies:
        features = {}
        if np.count_nonzero(sample_frequencies) > 0:
            features[f'{prefix}-DIST'] = np.count_nonzero(sample_frequencies)
            features[f'{prefix}-MEAN'] = np.mean(sample_frequencies)
            features[f'{prefix}-QMEAN'] = np.sqrt(np.mean(sample_frequencies**2))
            features[f'{prefix}-SUMSQ'] = np.sum(sample_frequencies**2)
            features[f'{prefix}-VAR'] = np.var(sample_frequencies)
            features[f'{prefix}-PVAR'] = np.var(sample_frequencies, ddof=0)
            features[f'{prefix}-STD'] = np.std(sample_frequencies)
            features[f'{prefix}-PSTD'] = np.std(sample_frequencies, ddof=0)
...
all_features = []
for chunk in chunks:
    chunk = chunk[['Domain', 'Target']]  # 只取需要的列
    print(chunk['Domain'].head())  # 输出前5行数据，确认是否有有效内容

    # 计算基本特征
    features_chunk = calculate_features_for_chunk(chunk)
    chunk = chunk.dropna(subset=['Domain'])

    # 提取n-gram特征
    unigrams_matrix = unigrams.fit_transform(chunk['Domain'])
    bigrams_matrix = bigrams.fit_transform(chunk['Domain'])
    trigrams_matrix = trigrams.fit_transform(chunk['Domain'])
    ...
    ngrams_features_df = pd.concat([unigrams_features_df, bigrams_features_df, trigrams_features_df], axis=1)

    # 合并特征
    all_features.append(pd.concat([chunk, features_chunk, ngrams_features_df], axis=1))

# 合并所有批次的结果
df_final = pd.concat(all_features, axis=0, ignore_index=True)
